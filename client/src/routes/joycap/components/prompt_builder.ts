...
export const presetConfigs: PromptConfig[] = [
    ...
    },
    {
        ...
        description: "Technical analysis of photographic elements including camera settings, lighting, and composition.",
        options: {
            ...defaultOptions,
...
export function buildExtraInstructions(options: PromptOptions): string[] {
    ...
    if (options.exclude_unchangeable_attributes) {
        instructions.push("Do NOT include information about people/characters that cannot be changed (like ethnicity, gender, etc), but do still include changeable attributes (like hair style)");
    }
    ...
    if (options.include_camera_details) {
        instructions.push("If it is a photo you MUST include information about what camera was likely used and details such as aperture, shutter speed, ISO, etc");
    }
    if (options.keep_pg) {
    ...
    if (options.include_quality_assessment) {
        instructions.push("You MUST include information about the subjective aesthetic quality of the image from low to very high");
    }
    if (options.include_composition) {
        instructions.push("Include information on the image's composition style, such as leading lines, rule of thirds, or symmetry");
    }
    if (options.exclude_text) {
        instructions.push("Do NOT mention any text that is in the image");
    }
    if (options.include_depth_of_field) {
        instructions.push("Specify the depth of field and whether the background is in focus or blurred");
    }
    if (options.include_lighting_source) {
        instructions.push("If applicable, mention the likely use of artificial or natural lighting sources");
    }
