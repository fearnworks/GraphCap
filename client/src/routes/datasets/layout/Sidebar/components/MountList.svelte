...
<div class="">
    <DropdownMenu.Root>
        ...
        <DropdownMenu.Content class="w-[250px] p-2 bg-slate-100">

            {#each datasetState.mounts as mount}
                <DropdownMenu.Item 
                    class="flex items-center justify-between rounded-md px-3 py-2 hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground cursor-pointer"
                ...
                >
                    ...
                    {#if selectedMountId === mount.id}
                        <Check class="h-4 w-4 ml-2 flex-shrink-0" />
                    {/if}
                </DropdownMenu.Item>
            {/each}
