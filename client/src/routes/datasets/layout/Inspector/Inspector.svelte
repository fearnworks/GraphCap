...
<div class="min-w-[40%] max-w-[40%] overflow-auto rounded-lg border bg-white">
	{#if datasetState.selectedImage}
		<Tabs value="image-details">
			...
			<TabsContent value="graph-caption">
				<GraphCaption {datasetState} />
			</TabsContent>
			<TabsContent value="image-details">
				<ImageDetails {datasetState} />
			</TabsContent>
			<TabsContent value="vqa-reasoner">
				<VQAReasoner {datasetState} />
			</TabsContent>
		</Tabs>
	{:else}
		<div class="p-4 text-center text-gray-500">Select an image to view details</div>
	{/if}
</div>
