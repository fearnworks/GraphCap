		...
		use:enhance={() => {
			...
			return async ({ result }) => {
				...
				if (result.type === 'success' && 'data' in result && result.data?.reasoning) {
					...
					const errorMessage =
						'data' in result && result.data?.error
	...
	<div class="space-y-4">
		{#if existingReasoning.length > 0}
			{#each existingReasoning as reasoning}
				<Card class="p-4">
					<Accordion.Root type="multiple" value={['question', 'response']} class="w-full">
						<Accordion.Item value="question" class="border-dark-10 border-b">
							<Accordion.Header>
						...
						<Accordion.Item value="analysis" class="border-dark-10 border-b">
							<Accordion.Header>
								<Accordion.Trigger class="flex w-full justify-between py-2">
							...
							<Accordion.Content class="accordion-content">
								<div class="grid grid-cols-2 gap-4">
									<div>
										...
										<p class="formatted-text text-sm text-muted-foreground">
											{reasoning.problemAnalysis}
										</p>
									</div>
									<div>
						...
						<Accordion.Item value="solution" class="border-dark-10 border-b">
							<Accordion.Header>
								<Accordion.Trigger class="flex w-full justify-between py-2">
							...
							<Accordion.Content class="accordion-content">
								<div class="space-y-3">
									<div>
										...
										<p class="formatted-text text-sm text-muted-foreground">
											{reasoning.solutionOutline}
										</p>
									</div>
									<div>
										<h4 class="section-title">Solution Plan</h4>
										<p class="formatted-text text-sm text-muted-foreground">
											{reasoning.solutionPlan}
										</p>
									</div>
								</div>
							</Accordion.Content>
						...
						<Accordion.Item value="response" class="border-dark-10 border-b">
							<Accordion.Header>
								<Accordion.Trigger class="flex w-full justify-between py-2">
		...
		{:else}
			<div class="rounded-lg bg-muted p-8 text-center">
				...
				<p class="mt-1 text-sm text-muted-foreground">Ask a question to get started.</p>
			</div>
		{/if}
	</div>
</div>
