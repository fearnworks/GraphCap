...
export async function getImageWithAnnotations(imageId: string): Promise<AggregatedImage | null> {
	// Get base image info
	const [baseImage] = await db.select().from(imageInfo).where(eq(imageInfo.id, imageId));
...
export async function getImagesWithAnnotations(imageIds: string[]): Promise<AggregatedImage[]> {
	const results = await Promise.all(imageIds.map((id) => getImageWithAnnotations(id)));
...
export async function getLatestImagesWithAnnotations(
	limit: number = 10
): Promise<AggregatedImage[]> {
	const latestImages = await db.select().from(imageInfo).orderBy(imageInfo.createdAt).limit(limit);
...
export async function getImagesByMountWithAnnotations(mountId: string): Promise<AggregatedImage[]> {
	const mountImages = await db.select().from(imageInfo).where(eq(imageInfo.mountId, mountId));
