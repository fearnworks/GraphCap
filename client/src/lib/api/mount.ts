...
export const listFiles = async ({ mountId, path }: { mountId: string; path: string }) => {
	try {
		...
		const [indexedFiles, unindexedFiles] = dirEntries.reduce(
			(acc, dirent) => {
...
export const getIndexingStatus = async ({ mountId, path }: { mountId: string; path: string }) => {
	...
	try {
		const indexedFiles = await db.query.imageInfo.findMany({
			where: and(eq(imageInfo.mountId, mountId), eq(imageInfo.relativePath, path))
		});
		...
		const allFiles = readdirSync(fullPath, { withFileTypes: true }).filter(
