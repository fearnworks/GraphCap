...
export async function saveCaption(captionData: ImageData) {
    const [imageDataEntry] = await db.insert(imageDataTable)
        .values({
    ...
    const savedTags = await db.insert(imageTagTable)
        .values(tagValues.map(tag => ({
            ...tag,
...
export async function getImageWithCaption(imageId: number) {
    const result = await db.query.imageDataTable
        .findFirst({
...
export async function getAllImages() {
    return db.query.imageDataTable
        .findMany({
            with: {
                tags: true
        },
...
export async function saveChainOfThought(data: {
    ...
    const [result] = await db.insert(chainOfThoughtTable)
        .values(data)
        .returning();
